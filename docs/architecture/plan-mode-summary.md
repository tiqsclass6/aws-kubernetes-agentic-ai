...
This document is the filled Plan Mode record for the live lab. It follow: clarification first, then research, then an eight-part plan, then review, then implementation only after approval.

Google Cloud | GKE Dataplane V2 | Vertex AI / Gemini | OPA | Cloud KMS | MCP | GitHub Actions

Updated 20 August 2026. Scripts `01`–`15`. Terraform `01`–`26`. Fail-closed defaults remain.

| **Field**           | **Value**                                                                                        |
| ------------------- | ------------------------------------------------------------------------------------------------ |
| **Project type**    | Student portfolio lab with production-minded architecture and security controls                  |
| **Current status**  | Lab cluster `vertex-agent-lab` in `us-central1-c` when provisioned; remediation remains disabled |
| **Complexity**      | 9/10                                                                                             |
...
8. **Blockers.** Git Bash `bq.cmd` / `\r` issues, kubectl impersonation cache, org-policy member domains, DESTROYED KMS versions 1 and 2 — which of these are live constraints?
...
| **Cloud and region**           | Project `class-6-5-tiqs`. Region `us-central1`. Zone `us-central1-c` (retry after `us-central1-b` stockout and `us-central1-f` CREATE_CLUSTER hang). Cluster `vertex-agent-lab`. Node type `e2-standard-4`. Datapath Dataplane V2, not Calico.                                              |
| **Core stack**                 | Terraform `01`–`26`, GKE, Workload Identity, Pub/Sub, Vertex AI / Gemini 2.5 Flash, OPA/Rego, Cloud KMS (approval signing version **3**), Firestore, BigQuery, Secret Manager, Cloud Logging, Cloud Monitoring / Managed Prometheus, Docker, GitHub Actions WIF, Trivy, Syft, Cosign.       |
...
| `terraform/01`–`26`                        | GCP foundation; includes CIS/Prowler hardening in `26-cis-hardening.tf`  |
...
- [x] Number scripts `01`–`15` and Terraform `01`–`26`.
...
| **Approval signature rejected**               | Canonical JSON, request hash, **key version 3**, reviewer, expiry.                        |
...
```text
You are acting as Cursor AI in PLAN MODE. I need you to follow this workflow:
...
MY REQUEST: Maintain the governed agentic security platform on GKE in `class-6-5-tiqs`. Preserve fail-closed defaults, numbered scripts `01`–`15`, Terraform `01`–`26`, KMS approval version 3, and the identity split between Gmail Owner, Workspace signer `admin@tiqsapp.com`, and `evidence-admin`. Do not enable controlled remediation without explicit approval.
...
4. **Cluster facts.** Zone `us-central1-c`, `e2-standard-4`, Dataplane V2, image tag `v1`, KMS approval version `3`.
...
| **Sensitive change control**   | Cloud KMS version 3 binds a Workspace reviewer to an exact request hash and expiry.           |
