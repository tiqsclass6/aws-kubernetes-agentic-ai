...
usage() {
  error "Usage: $0 approve|deny REVIEWER 'reason with at least 10 characters'"
  exit 2
}
...
if [[ -z "${PROJECT_ID}" || "${PROJECT_ID}" == "(unset)" ]]; then
  die "PROJECT_ID is not configured."
fi

TMP_DIR="$(mktemp -d)"
trap 'rm -rf "${TMP_DIR}"' EXIT

section "11  Review approval request"
...
action "Confirm before a signed ${DECISION^^} decision is published."
read -r -p "Publish signed ${DECISION^^} decision for this request? [y/N] " CONFIRM
if [[ ! "${CONFIRM}" =~ ^[Yy]$ ]]; then
  warn "Review cancelled; request was not acknowledged."
  exit 1
fi

section "Sign ${DECISION} decision with Cloud KMS"
...
PY_SIGNATURE
success "KMS signature attached."

section "Publish decision and acknowledge request"
...
success_section "Signed approval decision published"
log "Request acknowledged on ${SUBSCRIPTION}."
