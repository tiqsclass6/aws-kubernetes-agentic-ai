...
variable "node_machine_type" {
  description = <<-EOT
    ...
    no room even for broken-app's 100m request. e2-standard-2 still ran short once the
    full agent fleet was deployed simultaneously. e2-standard-4 keeps that headroom and
    is less prone to ZONE_RESOURCE_POOL_EXHAUSTED than n2-standard-2 in us-central1-b.
  EOT
  type        = string
...
variable "pipeline_backlog_threshold" {
  description = "Undelivered Pub/Sub messages that trigger the backlog alert"
  type        = number
  default     = 100
}
...
variable "policy_id" {
  description = "Policy identifier accepted by the governance and remediation agents"
  type        = string
  default     = "remediation"
}

variable "approval_signer_members" {
...
variable "evidence_retention_days" {
  description = "Default BigQuery table retention for governance evidence"
  type        = number
  default     = 90
}

variable "approval_backlog_threshold" {
  description = "Pending approval requests that trigger the review backlog alert"
  type        = number
  default     = 0
}
