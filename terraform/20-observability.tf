...
resource "google_monitoring_alert_policy" "pipeline_backlog" {
  project      = var.project_id
  display_name = "Security pipeline backlog"
  combiner     = "OR"
  enabled      = true

  documentation {
    content   = "One or more Pub/Sub subscriptions has a sustained message backlog. Inspect agent health, logs, and the dead-letter subscription."
...
resource "google_monitoring_alert_policy" "oldest_unacked_message" {
  project      = var.project_id
  display_name = "Oldest pipeline message"
  combiner     = "OR"
  enabled      = true
...
resource "google_monitoring_alert_policy" "dead_letter_messages" {
  ...
  documentation {
    content   = "The dead-letter subscription contains one or more events. Pull and inspect agent-events-dlq-sub before replaying messages."
    mime_type = "text/markdown"
  }
...
resource "google_monitoring_dashboard" "security_pipeline" {
  ...
  dashboard_json = jsonencode({
    displayName = "Agentic Security Operations"
    mosaicLayout = {
      columns = 12
      tiles = [
        {
